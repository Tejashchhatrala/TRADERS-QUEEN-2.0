...
export const Testimonials = () => {
  ...
  return (
    <section id="reviews" className="py-16 md:py-24 px-6 bg-surface-base">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <p className="text-brand-red font-bold tracking-widest uppercase text-sm mb-4">Real Traders</p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((review, idx) => (
              ...
              className="bg-surface-raised p-8 rounded-3xl border border-border-subtle shadow-lg relative flex flex-col h-full"
            >
              <div className="absolute top-6 right-6 text-border-subtle">
              ...
              <div className="flex items-center gap-4 border-t border-border-subtle pt-6 mt-auto">
                <div className="w-10 h-10 rounded-full bg-surface-elevated border border-border-strong flex items-center justify-center text-white font-bold uppercase text-sm">
                  {review.name.charAt(0)}
                </div>
                <div>
                  <h4 className="text-white font-bold text-sm flex items-center gap-2">
                    {review.name}
                    <span className="text-brand-teal text-[10px] uppercase font-bold tracking-wider flex items-center gap-1 bg-brand-teal/10 px-1.5 py-0.5 rounded border border-brand-teal/20 relative group cursor-default">
                      <Star size={8} fill="currentColor" /> Verified
                      {/* Tooltip */}
                      <span className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-36 bg-surface-raised text-text-base text-[9px] p-2 rounded border border-border-strong z-10 normal-case tracking-normal text-center shadow-xl">
                        WhatsApp review on file
                      </span>
                    </span>
                  </h4>
                  <p className="text-text-subtle text-xs font-mono mt-0.5">{review.role}</p>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      <div className="max-w-6xl mx-auto mt-32">
        <div className="text-center mb-16">
          <p className="text-brand-teal font-bold tracking-widest uppercase text-sm mb-4">Results</p>
          <h2 className="text-4xl md:text-5xl font-bold mb-6 text-white">Profits made by traders using TQ 2.0 </h2>
          <p className="text-text-muted max-w-2xl mx-auto">Screenshots submitted by our users from their verified broker accounts like Upstox and Zerodha showing consistent returns.</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pb-10">
          {[1,2,3,4,5,6,7,8].map((num) => (
             <div key={num} className="bg-surface-raised border border-border-subtle rounded-2xl aspect-[4/5] relative overflow-hidden group">
               <img 
                 src={`/profit-${num}.jpg`}
                 alt={`Trader Profit Proof ${num}`}
                 className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-105 transition-all duration-500 bg-surface-base"
                 onError={(e) => {
               ...
               <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent p-4 flex flex-col justify-end h-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                 <div className="flex items-center gap-1.5 text-brand-teal text-[10px] font-bold uppercase tracking-widest">
